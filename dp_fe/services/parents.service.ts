...
import type { Parent } from "@/types/models/parent";
...
export const parentsService = {
  create(payload: CreateParentPayload) {
    return axiosInstance
      .post(endpoints.parents, payload)
      .then((r) => r.data.data as Parent);
  },

  list(q?: string) {
    return axiosInstance
      .get(endpoints.parents, { params: { q } })
      .then((r) => r.data.data as Parent[]);
  },

  update(id: string, payload: Partial<CreateParentPayload>) {
    return axiosInstance
      .patch(`${endpoints.parents}/${id}`, payload)
      .then((r) => r.data.data as Parent);
  },

  remove(id: string) {
    return axiosInstance
      .delete(`${endpoints.parents}/${id}`)
